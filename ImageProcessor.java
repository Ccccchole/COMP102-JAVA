...
public class ImageProcessor{
     ...
     * but make sure that you never go below 0 (minimum level - black).
     */
    ...
    public void contrastImage(){
        for(int row = 0; row<this.image.length; row++){
            for(int col = 0; col<this.image[0].length; col++)
            {
                ...
                {
                    this.image[row][col] = this.image[row][col]+(this.image[row][col]-128)*20/100;
                }
                else if(this.image[row][col]<=128&&this.image[row][col]-(128-this.image[row][col])*20/100>=0)
                {
                    this.image[row][col] = this.image[row][col]-(128-this.image[row][col])*20/100;
                }
            }
        }
        this.redisplayImage();
    ...
    public void flipImageHorizontally(){
        for(int row = 0; row<this.image.length; row++){
            for(int col = 0; col<=this.image[0].length/2; col++)
            {
                ...
                this.image[row][col] = this.image[row][this.image[0].length-1-col];
                this.image[row][this.image[0].length-1-col] = temp;
            }
        }
        this.redisplayImage();
    }
    ...
    public void flipImageVertically(){
        for(int col = 0; col<this.image[0].length; col++){
            ...
            {
                int temp = image[row][col];
                this.image[row][col] = this.image[this.image.length-1-row][col];
                this.image[this.image.length-1-row][col] = temp;
            }
        }
        this.redisplayImage();
    }
    ...
    public void rotateImage180(){
        int[][] oneEighty = new int[this.image.length][this.image[0].length];
        for(int row = 0; row<this.image.length; row++){
            for(int col = 0; col<this.image[0].length; col++)            
            {
                oneEighty[row][col]=this.image[this.image.length-1-row][this.image[0].length-1-col];
            }
        }
        this.image = oneEighty;
        this.redisplayImage();
    }
    ...
    public void rotateImage90(){
        int[][] ninety = new int[this.image[0].length][this.image.length];
        for(int row = 0; row<this.image[0].length; row++){
            for(int col = 0; col<this.image.length; col++)            
            {
                ninety[row][col]=this.image[col][this.image[0].length-1-row];
            }
        }
        this.image = ninety;
        this.redisplayImage();
    ...
    public void expandImage(){
        int[][] quarter = new int[this.image.length][this.image[0].length];
        for(int row = 0; row<this.image.length; row=row+2){
            for(int col = 0; col<this.image[0].length; col=col+2)
            {
                if(row+1<this.image.length&&col+1<this.image[0].length)
                {
                    quarter[row][col]=this.image[row/2][col/2];
                    quarter[row][col+1]=quarter[row][col];
                    quarter[row+1][col]=quarter[row][col];
                    quarter[row+1][col+1]=quarter[row][col];
                }
            }
        }
        this.image = quarter;
    ...
    public void zoomImage(){
        int[][] zoomIn = new int[this.image.length*133/100][this.image[0].length*133/100];
        //Expend the image by 133%:
        for(int row = 0; row<this.image.length*133/100; row++)
        {
            for(int col = 0; col<this.image[0].length*133/100; col++)
            {
               
                
                    zoomIn[row][col]=this.image[row*3/4][col*3/4];
                
            }
        }        
        //Make it centered on the selected position:
        for(int row = 0; row<this.image.length; row++)
        {
            for(int col = 0; col<this.image[0].length; col++)
            {
            this.image[row][col]=zoomIn[row+selectedRow/3][col+selectedCol/3]; 
        }
        }
        this.redisplayImage();
    }

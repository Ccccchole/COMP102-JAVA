...
public class PowerAnalyser{
    ...
    public void analyseLevels() {
        ...
        UI.print("Please enter all the levels."); 
        UI.println();
        double sum = 0;
        int count = 0;
        ...
        UI.print("-------Analysis-------");
        UI.println();
        UI.print("Max level: "+ max +"kWh");
        UI.println();
        UI.print("Mix level: "+ min +"kWh");
        UI.println();
        UI.print("Average level: "+ average +"kWh");
    ...
    public void plotLevels() {
        ...
        int MAX = UI.askInt("Max level?");
        UI.drawLine(10, 450, 1000, 450);
        UI.drawLine(10, 450-MAX/20, 10, 450);
        UI.drawString("8000", 10, 50);
        UI.drawString("4000", 10, 250);
        int xLabel = 30;
        while(xLabel<1000)
        {
            UI.drawString("|", xLabel, 450);
            xLabel = xLabel + 20;
        }
        int yLabel = 450-MAX/20;
        while(yLabel<450)
        {
            UI.drawString("-", 10, yLabel);
            yLabel = yLabel + 50;
        }
        UI.print("Please enter all the levels."); 
        UI.println();
        ...
        {
            int num = UI.nextInt();
            UI.drawRect(x,450-num/20, 15, num/20);
            if(count>24){
                break;
            };
            if(num>8000){
                UI.drawString(" *", x, 448-num/20);
            };
            x=x+20;
            count++;
        }

...
public class WordGridExercise{
    ...
    public void redisplay(){
        ...
        for(int row = 0; row<this.wordGrid.length; row++)
        {
            ...
            {
                UI.drawRect(this.left+this.wd*col, this.top+this.ht*row, this.wd, this.ht);
                if(this.wordGrid[row][col]!=null)
                {
                    UI.drawString(this.wordGrid[row][col], this.left+this.wd*col, this.top+this.ht*(row+1));
                }
            }
            UI.println();
        }
